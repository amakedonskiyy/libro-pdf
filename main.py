...
ENGINE_VERSION = "2026-06-30-editor-default-v1"
...
def _job_time_budget(pdf_bytes, proofread=False):
    """Адаптивний потолок часу: масштаб від числа сторінок, підлога/стеля.
    proofread -> другий прохід (редактор), часу вдвічі більше.
    ...
    try:
        ...
        if proofread:
            budget *= 2          # редактор = другий прохід LLM
...
def _run_local_job(job_id, pdf_bytes, api_key, provider, model, src, dst,
    ...
    try:
        ...
        JOBS[job_id]["total_limit"] = _job_time_budget(pdf_bytes, proofread)  # адаптивний потолок часу
